export function markerClick(marker, obj, isHistory, that) {//点击标注时触发事件 isHistory：true--是上次标记的点
    //var that=this;
    // console.log($("#LAY-auth-tree-index .auth-single").length)
    // if($("#LAY-auth-tree-index .auth-single").length>0){
...
import dialog from './components/wgsjDialog.vue'
/**
 * 绘制点 线 面 的时候触发， 并弹框
 */
 ...
 * 绘制矩形
 */
export function openRectangleTool(that) {
    var handler = that.handler;
    if (handler) handler.close();
 ...
 * 绘制圆
 */
export function openCircleTool(that) {
    var handler = that.handler;
    if (handler) handler.close();
