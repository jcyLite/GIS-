...
<template>
  <div id="app">
    ...
    >
      ...
      <div class="bottom">
        ...
        <div class="container">
          <div
            ...
            @click="clickLeftLayer(item,index)"
            class="box"
            :tid="item.tid"
          ...
          >
            <div class="t">
              ...
              <div class="contenteditable too-long1">{{item.tname }}</div>
              <div class="u">
                <img
                  @click.stop="changeName(item,index)"
                  src="./img/edit.png"
                  alt
                  style="margin-right:6px;"
                >
                <img
                  @click="poperDetail(item,index)"
                  src="./img/detail.png"
                  alt
                ...
                <img
                  @click.stop="deleteLayer(item,index)"
                  src="./img/hsz.png"
                  alt
                  style="width: 15px;"
                >
      ...
      <div ref="toright" class="right"></div>
    </div>
...
export default {
  ...
  methods: {
    ...
    clickEye(item, index) {
      ...
      this.eye.indexOf(index) == -1
        ? this.eye.push(index)
        : this.eye.splice(this.eye.indexOf(index), 1);
    ...
    setCenterMap() {
      ...
      window.map.centerAndZoom(
        new T.LngLat(centerPoint.lng, centerPoint.lat),
        centerzoom
      );
    },
    /**
    ...
    markerClick(marker, obj, isHistory) {
      // isHistory：true--是上次标记的点
      //点击标注时触发事件
      //alert('22'+isHistory)
    ...
    //         that.$children.forEach((item, index) => {
    //           // 选择poperBottom组件实例,修改oactive
    ...
    //       });
    //       marker.addEventListener("dragend", function(param) {
    //         // 拖拽事件
